...
Same scanner as before, but it also runs a tiny web server. Render's free tier
only hosts "web services" (which must listen on a port), not background workers,
so this lets the bot qualify for free hosting. A keep-alive pinger hitting the
service URL every few minutes stops Render from sleeping it after 15 min idle.

Public endpoints used (no API key, ~60 req/min limit):
...
def alert(profile: dict, matched: list) -> None:
...
def save_seen(seen: set) -> None:
    try:
        ...
        print(f"[warn] could not save seen file: {e}")
...
def scanner_loop() -> None:
    ...
    print(
        f"Scanning {', '.join(sorted(CHAINS))} for summaries containing "
        f"{KEYWORDS} every {POLL_SECONDS}s.",
        flush=True,
    )
    while True:
        try:
...
def main() -> None:
    ...
    # Run the web server in the foreground (Render watches this port).
